...
//void Draw_Coordinate(u16 distance, u16 colour)
//{
//		POINT_COLOR = colour;
//		LCD_DrawLine(10, 0, 10, lcddev.height);
//		LCD_DrawLine(0, 230, 250, 230);
//	
//		for(int i=0; i<lcddev.height/(distance); i++)
//			{LCD_DrawLine(5,230-distance*i, 250, 230-distance*i);}
//	
//		for(int i=0; i<260/(distance); i++)
//			{LCD_DrawLine(10+distance*i,0, 10+distance*i, 235);}
//}
...
void Draw_Coordinate(u16 distance, u16 colour)
{
	...
	//	LCD_DrawLine(0, 230, 250, 230);
		...
		for(int i=0; i<200/(distance); i++)
			{
				LCD_DrawLine(0,240-distance*i, 320, 240-distance*i);
				LCD_ShowNum(1, 240-distance*i, i, 1, 12);
			}
	
		for(int i=0; i<320/(distance); i++)
			{
				LCD_DrawLine(distance*i,60, distance*i, 240);
				LCD_ShowNum(distance*i, 230, i, 2, 12);
			}
}
/*------------------------------------------------*/

void Draw_Wave_Line(u16 x, u16 y1, u16 y2)
{
	if((y1 < y2) &&(y2-y1)<=20)
	{
		u16 average = (u16)(y2 + y1)/2.0;
		
		for(u8 i=0; i<= (average-y1); i++)			
				LCD_DrawPoint(x,y1+i);
			
		for(u8 i=0; i<= (y2-average); i++)			
				LCD_DrawPoint(x+1, average+i);		
			
	}
	
	else if((y1 > y2) &&(y1-y2)<=20 )
	{
		u16 average = (u16)(y1 + y2)/2.0;
		
		for(u8 i=0; i<= (y1-average); i++)			
				LCD_DrawPoint(x,average+i);
			
		for(u8 i=0; i<= (average-y2); i++)			
				LCD_DrawPoint(x+1, average-i);		
	...
	{
		LCD_DrawPoint(x,y1);
		LCD_DrawPoint(x+1,y2);
	}
	
	else	LCD_DrawLine(x,y1,x+1,y2);
